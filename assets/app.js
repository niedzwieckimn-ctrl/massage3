...
function handleSubmit(e){
  ...
  // === SUPABASE: klient + booking + oznacz slot + odśwież LS ===
try {
  ...
  // UWAGA: w <select id="service"> MUSI być value = UUID usługi
  const service_id = document.getElementById('service')?.value;

  // UWAGA: w <select id="time"> MUSI być value = UUID slotu
  const slot_id    = document.getElementById('time')?.value;

  if (!service_id || !slot_id) {
    alert('Wybierz zabieg i godzinę.');
    return;
  }

  // 2) klient
  const client_id = await sbEnsureClient({ name, email, phone, address });
  if (!client_id) {
    alert('Nie udało się zapisać klienta.');
    return;
  }

  // 3) booking + zajęcie slotu (atomowo)
  const res = await sbCreateBookingAndTakeSlot({ client_id, service_id, slot_id, notes });

  if (!res.ok && res.reason === 'taken') {
    alert('Wybrany termin został właśnie zajęty. Wybierz inny.');
    return;
  }
  if (!res.ok) {
  ...
  // 4) odśwież wolne sloty (żeby kalendarz pokazał aktualny stan)
  await sbRefreshFreeSlotsToLS();

  // 5) dalej po staremu: baner „Dziękujemy” / e-mail do masażystki / reset form itp.
  // (zostaw Twój dotychczasowy kod 'thank you' i sendEmail)
